...
func networkWithClassObjects(t *testing.T, n int) (*network.Network, []types.Class) {
	t.Helper()
	...
	return network.New(t, cfg), state.ClassList
}

func TestShowClass(t *testing.T) {
	net, objs := networkWithClassObjects(t, 2)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdShowClass(), args)
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetClassResponse
...
func TestListClass(t *testing.T) {
	net, objs := networkWithClassObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListClass(), args)
			require.NoError(t, err)
			var resp types.QueryAllClassResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
