...
func networkWithPowerupObjects(t *testing.T, n int) (*network.Network, []types.Powerup) {
	t.Helper()
	...
	for i := 0; i < n; i++ {
		...
		nullify.Fill(&powerup)
		state.PowerupList = append(state.PowerupList, powerup)
	}
	buf, err := cfg.Codec.MarshalJSON(&state)
	require.NoError(t, err)
	cfg.GenesisState[types.ModuleName] = buf
	return network.New(t, cfg), state.PowerupList
}

func TestShowPowerup(t *testing.T) {
	net, objs := networkWithPowerupObjects(t, 2)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdShowPowerup(), args)
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetPowerupResponse
				require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
				require.NotNil(t, resp.Powerup)
				require.Equal(t,
					nullify.Fill(&tc.obj),
...
func TestListPowerup(t *testing.T) {
	net, objs := networkWithPowerupObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListPowerup(), args)
			require.NoError(t, err)
			var resp types.QueryAllPowerupResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
